# Priscilla Fx Price List

[Связаться с нами: https://t.me/priscilla_ef](https://t.me/priscilla_ef)

Обратитесь к нам за уникальным заказом, если не нашли что искали.

| Категория            | Название                                          | Цена (₽)             |
| -------------------- | ------------------------------------------------- | -------------------- |
| **Repair**           | ZOOM MS-50G (+ LED)                               | Уточнять в переписке |
|                      | Digitech Digidelay (+ LED)                        |                      |
|                      |                                                   |                      |
| **Mods: Hi-Fi (TK)** | BOSS DA-2 Hi-Fi mod                               | 2800 ₽               |
|                      | BOSS DD-20 Hi-Fi mod                              | 2800 ₽               |
|                      | BOSS OD-3 Hi-Fi mod                               | 2800 ₽               |
| **Mods: TK**         | BOSS LS-2 TK mod                                  | 4800 ₽               |
|                      | BOSS OD-3 TK mod (SPI)                            | 6800 ₽               |
|                      | BOSS SPH-1 TK mod (WAXX + SPI)                    | 8800 ₽               |
|                      |                                                   |                      |
| **Copy**             | Vemuram Jan Ray                                   | - ₽                  |
|                      | Way Huge Red Llama 25th                           | - ₽                  |
|                      | Xotic EP Booster                                  | - ₽                  |
|                      | Fulltone OCD v1.2                                 | - ₽                  |
|                      | Maxon OD-820                                      | 13800 ₽              |
|                      |                                                   |                      |
| **Custom**           | Priscilla FX「LS-SR」3PDT buffered bypass         | 2800 ₽               |
|                      | Priscilla FX「LS-SR A/B Box」3PDT buffered bypass | - ₽               |
|                      | Priscilla FX「HB Booster」                        | 4800 ₽               |
|                      | Priscilla FX「M-Umineko」                         | 16800 ₽              |
|                      | Priscilla FX「TS-Special」                        | 16800 ₽              |
|                      | Priscilla FX「PRISCILLA」                         | 16800 ₽              |

## Памятка

- **В некоторых случаях мы не сможем выполнить ваши пожелания.**

- Учтите, что в процессе модификации на корпусе могут появиться небольшие повреждения.  

- Покрытие, нанесенное вручную, не обладает идеальной поверхностью, что может приводить к адгезии мелкодисперсной пыли, присутствующей в воздушной среде, а также к образованию воздушных пузырьков.

- Мы выполняем только простые ремонтные работы, например, замену переключателя.

- Оплата производится после завершения работ, перед отправкой.  

- Доставка осуществляется клиентом, обратная доставка включена в стоимость.

- Hi-Fi моды не изменяют специфику схемы и важные значения компонентов.

- SPI, WAXX, WEED эквивалентны друг другу в категории Hi-FI.
