## Mods:

### Без изменения оригинальной специфики схемы (Hi-Fi):

  ※ TK имеет **SPI**, **WAXX**. В категории Hi-Fi - SPI, WAXX, WEED, PFX эквивалентны друг другу.

### Изменение специфики схемы + Hi-Fi:
- BOSS **LS-2 TK** mod (SPI = WAXX)
- BOSS **OD-3 TK** mod (SPI)
- BOSS **SPH-1 TK** mod (SPI + WAXX in one with 2 toggle switches)

---

## Custom
- **Priscilla FX 「LS-SR」 v1**

  ※ Line Selector, 3PDT button, no additional functionality

- **Priscilla FX 「HB Booster」(9-18v)**

  ※ Hot Drive'n'Boost MK3 booster channel copy, 18v is canonical for MK3
  
- **Priscilla FX 「Maxon OD-820」**

  ※ Maxon copy

- **Priscilla FX「M-Umineko」**

  ※ Modern Overdrive

- **Priscilla FX「TS-Special」**

  ※ Special Overdrive

- **Priscilla FX「PRISCILLA」**

  ※ Overdrive-Distortion

---
